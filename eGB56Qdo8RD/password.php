...
if(isset($_GET['action'])){
	if($_GET['action'] == 'edit'){
		...
		$old_pass=md5(md5(trim($_POST['old_pass']))."886");
		$new_pass=md5(md5(trim($_POST['new_pass']))."886");
		$count = $database->count(DB_PREFIX.'manage',array('AND'=>array('username'=>$current,'password'=>@$old_pass)));
		if($count > 0){
			 $database->update(DB_PREFIX.'manage',array('password'=>@$new_pass),array('username'=>$current));
			 echo '{"stat":0}';
			return;
		}else{
			echo '{"stat":1}';
